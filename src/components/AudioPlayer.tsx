...
export default function AudioPlayer({ src, title }: AudioPlayerProps) {
  ...
  return (
    <div 
      ref={containerRef}
      className="group relative bg-white/[0.03] backdrop-blur-xl rounded-xl border border-white/5 p-3 hover:border-pink-500/25 hover:shadow-[0_0_20px_rgba(236,72,153,0.08)] transition-all duration-300 overflow-hidden"
    >
      {/* 微光效 */}
      <div className="absolute inset-0 bg-pink-500/0 group-hover:bg-pink-500/[0.03] transition-all duration-500" />
        ...
        <button
          ...
          className={`relative w-11 h-11 flex-shrink-0 flex items-center justify-center rounded-full text-white transition-all duration-300 ${
            error || !isVisible
              ...
              : 'bg-pink-500 hover:shadow-[0_0_15px_rgba(236,72,153,0.3)] hover:scale-110'
          }`}
        >
          {!isVisible ? (
          ...
          {isPlaying && (
            <div className="absolute inset-0 rounded-full bg-pink-400/20 animate-ping" />
          )}
          {/* 光环 */}
          <div className="absolute inset-0 rounded-full border border-pink-400/30 opacity-0 group-hover:opacity-100 transition-opacity" />
        </button>

        <div className="flex-1 min-w-0">
          <h3 className="font-medium text-white truncate text-sm group-hover:text-pink-400 transition-colors" title={title}>
            {title}
          </h3>
      ...
      <div className="relative">
        ...
        >
          <div
            className="h-full bg-pink-500 rounded-full transition-all duration-100 relative"
